...
const Section = ({ title, children }) => (
    <div className="mb-8 border-b border-black pb-4">
        <h3 className="text-sm font-bold uppercase tracking-wider mb-4 underline">
            {title}
        </h3>
...
const LabelValue = ({ label, value }) => (
    <div className="flex flex-col sm:flex-row sm:gap-4">
        <span className="font-bold min-w-[150px] uppercase text-[12px]">{label}:</span>
        <span className="text-[13px]">{clean(value)}</span>
    </div>
);
                ...
                >
                    ...
                    <div className="flex items-center justify-between px-8 py-4 border-b border-black bg-white z-10">
                        <div className="flex flex-col">
                            <h2 className="text-lg font-bold uppercase tracking-tight">TRUTHFLOW REPORT</h2>
                            <p className="text-[10px] opacity-70">CONFIDENTIAL DOCUMENT • FOR OFFICIAL USE ONLY</p>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar bg-white">
                        <div className="max-w-4xl mx-auto px-10 py-12">
                            ...
                            <div className="mb-12 border-b-2 border-black pb-6 text-center">
                                <h1 className="text-2xl font-black uppercase mb-1">INTELLIGENCE REPORT</h1>
                                <p className="text-sm">RECORDED BY: {clean(data.heading?.recorded_by)}</p>
                                <p className="text-sm">DATE/TIME: {data.heading?.recording_datetime ? new Date(data.heading.recording_datetime).toLocaleString() : "PENDING"}</p>
                            </div>
                                ...
                                {data.timeline?.map((event, idx) => (
                                    <div key={idx} className="mb-6 last:mb-0 border-l-2 border-black pl-4 py-1">
                                        <div className="flex flex-wrap gap-x-4 mb-2">
                                            <span className="font-bold underline text-[12px]">{event.event}</span>
                                            <span className="text-[11px] opacity-70 italic">[{clean(event.time) || "TIME UNKNOWN"}]</span>
                                            <span className="text-[11px] opacity-70 italic">@{clean(event.location)}</span>
                                        </div>
                                        <p className="text-[12px] mb-2">{event.description}</p>
                                        {event.people?.length > 0 && (
                                            <p className="text-[11px]"><span className="font-bold uppercase">People:</span> {event.people.join(', ')}</p>
                                        )}
                                    </div>
                                ))}
                            </Section>
                            ...
                            <Section title="Chain of Evidence">
                                ...
                                )) : <p className="text-[13px]">No evidence files attached.</p>}
                            </Section>
                    ...
                    {/* Footer */}
                    <div className="px-10 py-4 border-t border-black bg-white text-center">
